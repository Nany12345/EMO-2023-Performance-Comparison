...
%--------------------------------------------------------------------------
    ...
    methods(Access = protected)
        ...
        %        Algorithm = MOEAD('parameter',4,'save',1)
            ...
            for i = isStr(ismember(varargin(isStr),{'parameter','save','outputFcn','run'}))
    ...
    methods(Sealed)
        ...
        %       Algorithm.Solve(Problem)
            ...
            try
                obj.pro    = Problem;
                obj.result = struct('population',zeros(obj.pro.N,obj.pro.M,7), ...
                    'runtime',zeros(1,obj.pro.maxFE/obj.pro.N), ...
                    'UEA',zeros(obj.pro.maxFE,obj.pro.M));
    ...
    methods(Access = protected, Sealed)
        ...
        %           ... ...
        %       end
            genInd = [1,5,10,20,50,100,200];
            gen = floor(obj.pro.FE/obj.pro.N);
            if gen==1
                obj.result.runtime(1,gen) = toc;
            else
                obj.result.runtime(1,gen) = obj.result.runtime(1,gen-1) + toc;
            end
            if ~isempty(find(gen==genInd,1))
                obj.result.population(1:size(Population.objs,1),:,find(gen==genInd,1)) = Population.objs;
            end
            % drawnow(); 
            obj.outputFcn(obj,obj.pro);
            nofinish = obj.pro.FE < obj.pro.maxFE;
            if ~nofinish
                obj.result.UEA = obj.pro.UEA;
            end
        ...
        % The default output function of ALGORITHM.
            ...
            if Problem.FE >= Problem.maxFE
                ...
                elseif Algorithm.save > 0
                    ...
                    [~,~]  = mkdir(folder);
                    file   = fullfile(folder,sprintf('%s_%s_M%d_',class(Algorithm),class(Problem),Problem.M));
                    runNo  = Algorithm.run;
                    POP = Algorithm.result.population;
                    runtime = Algorithm.result.runtime;
                    save([file,num2str(runNo),'.mat'],'POP','runtime');
                    % save ref data
                    folder = fullfile('Data/ref',class(Algorithm));
                    [~,~]  = mkdir(folder);
                    file   = fullfile(folder,sprintf('%s_%s_M%d_',class(Algorithm),class(Problem),Problem.M));
                    UEA = Problem.UEA;
                    FrontNo = NDSort(UEA,1);
                    UEA = UEA(FrontNo==1,:);
                    save([file,num2str(runNo),'.mat'],'UEA');
                end
            end
        end
        function cb_menu(h)
