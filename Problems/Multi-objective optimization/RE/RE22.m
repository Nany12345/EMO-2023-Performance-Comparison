classdef RE22 < PROBLEM
% <multi/many> <real> <large/none> <expensive/none>
% a four bar truss design problem.
    ...
    methods
        ...
        function PopObj = CalObj(obj,PopDec)
            ...
            for i = 1:size(PopDec,1)
                x1 = closestValue(FEASIBLE_VALUES_RE22, PopDec(i,1));
                x2 = PopDec(i,2);
                x3 = PopDec(i,3);

                % First original objective function
                PopObj(i,1) = (29.4 * x1) + (0.6 * x2 * x3);

                % Original constraint functions 	
                g(1) = (x1 * x3) - 7.735 * ((x1 * x1) / (x2+1e-6)) - 180.0;
                g(2) = 4.0 - (x3 / (x2+1e-6));
        ...
        function R = GetOptimum(obj,N)
            R = load('reference_points_RE22.dat');
        end
    end
end